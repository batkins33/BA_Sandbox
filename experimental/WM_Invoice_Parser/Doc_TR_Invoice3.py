...
MAX_WORKERS = 16  # Tune this to your CPU (or try 2-8)
...
def ocr_page(idx_img_tuple):
    idx, img = idx_img_tuple
    try:
        result = model([img])
        print(f"\n==== RAW OCR RESULT, PAGE {idx + 1} ====")
        print(result)
        if hasattr(result, "pages") and result.pages:
            print(f"Export(): {result.pages[0].export()}")
        else:
...
with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
    future_to_idx = {executor.submit(ocr_page, (idx, img)): idx for idx, img in enumerate(pages)}
